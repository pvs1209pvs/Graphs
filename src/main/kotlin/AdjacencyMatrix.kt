...
class AdjacencyMatrix(graphSrc: List<List<String>>) {
     ...
     */
    fun getOutDeg(vertex: Int) = graph[vertex].count { it == 1 }

    fun getOutDeg(): List<Int> {

        val allOutDegs = mutableListOf<Int>()

        for (v in graph.indices) {
            allOutDegs += getOutDeg(v)
        }
    ...
    fun getInDeg(): List<Int> {

        val allInDegs = mutableListOf<Int>()

        for (v in graph.indices) {
            allInDegs += getInDeg(v)
        }
    ...
    /**
     * Eulerian Path = visits every edge exactly one.
     * EP has to start from a certain node.
     * EP cant start and end at the same node
     *
     * Eulerian Circuit = EP that starts and ends on the same vertex.
     * EC can start from anywhere.
     *
     * If we have an algo for EP then EC comes for free by passing EC graph to EP
     */

//    fun findInDeg(vertex: Int) = graph[vertex].count { it == 1 }
//

    /**
     * Index of the first vertex that has exactly count more out-degrees and in-degrees. Negative howMany returns the vertex that has exactly count more in-degrees than out-degrees.
     * @param count Difference of out and in degree.
     * @param outIn Out and In degree zipped together.
     **/
    fun degDiff(outIn: List<Pair<Int, Int>>, count: Int) = outIn.indexOfFirst { it.first - it.second == count }

    fun isEP(outIn: List<Pair<Int, Int>>): Pair<Int, Int>? {

        // make sure out-in and in-out is at most 1
        if (outIn.count { it.first - it.second == 1 } != 1) return null
        if (outIn.count { it.first - it.second == -1 } != 1) return null

        // get index of out-in and in-out
        val outMinusIn = degDiff(outIn, 1)
        val inMinusOut = degDiff(outIn, -1)

        var equalInOut = 0
        for ((i, v) in outIn.withIndex()) {
            if (i == outMinusIn || i == inMinusOut) continue
            if (v.first == v.second) ++equalInOut
        }

        // all other vertices have equal in and out degs
        return if (equalInOut == graph.size - 2) Pair(outMinusIn, inMinusOut) else null

    }

    fun epPath() {

        val outDegs = getOutDeg()
        val inDegs = getInDeg()
        println(outDegs)
        val start = isEP(outDegs.zip(inDegs))
        println("Start ${start?.first}")

        if (start == null) {
            println("no path")
        }

        epPath(start!!.first, outDegs as MutableList<Int>)

    }

    fun epPath(from: Int, outDegs: MutableList<Int>) {

        println(from)

        outDegs[from]--

        val neigh = graph[from]

        for (to in neigh.indices) {

            if (neigh[to] == 0) continue
            graph[from][to] = 0
            epPath(to, outDegs)
        }
